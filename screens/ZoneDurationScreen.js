...
export default function ZoneDurationScreen({ navigation }) {
  ...
  const loadDurations = async () => {
    try {
      const res = await api.get('/zonedurations');
      setDurations(res.data.durations || {});
    } catch (error) {
      console.error('Load zone durations error:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put('/zonedurations', { durations });
      Alert.alert(t('success'), t('zoneDurationSaved'), [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (error) {
      Alert.alert(t('error'), error.response?.data?.message || t('failedToSaveZoneDuration'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const setCount = () =>
    Object.values(durations).filter(v => v !== '' && v !== undefined && Number(v) > 0).length;

  const totalZones = ZONE_GROUPS.reduce((n, g) => n + g.zones.length, 0);
    ...
    <LinearGradient colors={theme.gradientBackground} style={styles.container}
      ...
      <LinearGradient colors={theme.gradientHeader}
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.headerTitle, { color: theme.textPrimary }]}>{t('zoneDuration')}</Text>
          {!loading && (
      ...
      {loading ? (
        ...
        <KeyboardAvoidingView style={{ flex: 1 }}
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}
            ...
            <Text style={[styles.hint, { color: theme.textSecondary }]}>{t('zoneDurationHint')}</Text>
            ...
            {ZONE_GROUPS.map(group => (
              <View key={group.titleKey} style={styles.groupWrapper}>
                <View style={styles.sectionHeader}>
                  ...
                  <Text style={[styles.sectionTitle, { color: theme.textPrimary }]}>{t(group.titleKey)}</Text>
                </View>
                ...
                <View style={[styles.card, { backgroundColor: theme.glassCard, borderColor: theme.glassCardBorder }]}>
                  {group.zones.map(({ key, icon }, index) => {
                    ...
                    const hasValue  = durations[key] !== undefined && durations[key] !== '' && Number(durations[key]) > 0;
                    return (
                      <View key={key} style={[
                        styles.row,
                        ...
                        <View style={[styles.zoneIcon,
                          { backgroundColor: hasValue ? theme.primary + '22' : theme.primary + '11' }]}>
                          <Ionicons name={icon} size={16} color={hasValue ? theme.primary : theme.textTertiary} />
                        </View>
                        ...
                        <View style={styles.inputGroup}>
                          <View style={[styles.inputWrapper, {
                            ...
                            ...(isFocused && { shadowColor: theme.primary, shadowOpacity: 0.18, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } }),
                          }]}>
                            <TextInput
                              style={[styles.input, { color: theme.textPrimary }]}
                              value={durations[key] !== undefined ? String(durations[key]) : ''}
                              onChangeText={val => setDuration(key, val)}
                          ...
                          <Text style={[styles.unit, { color: theme.textTertiary }]}>{t('minShort')}</Text>
                        </View>
                      </View>
                    );
                  })}
            ...
            <Text style={[styles.footerNote, { color: theme.textTertiary }]}>{t('zoneDurationFooter')}</Text>
            ...
            <TouchableOpacity
              ...
              <LinearGradient colors={theme.gradientPrimary} style={styles.saveButtonGradient}
                ...
                {saving
                  ...
                  : <Text style={[styles.saveButtonText, { color: isDark ? '#111' : '#fff' }]}>{t('saveZoneDurations')}</Text>}
              </LinearGradient>
            </TouchableOpacity>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: Platform.OS === 'ios' ? 56 : 36, paddingHorizontal: 20, paddingBottom: 16, borderBottomWidth: 1,
  },
  backBtn: { padding: 4 },
  headerCenter: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 8 },
  headerTitle: { fontSize: 18, fontWeight: '800', letterSpacing: -0.4, flexShrink: 1 },
  headerBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 },
  headerBadgeText: { fontSize: 12, fontWeight: '700' },
  saveBtn: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 10, minWidth: 60, alignItems: 'center' },
  saveBtnText: { fontSize: 15, fontWeight: '700' },
  centered: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  scrollContent: { padding: 20, paddingBottom: 48 },
  hint: { fontSize: 14, fontWeight: '500', lineHeight: 20, marginBottom: 24, paddingHorizontal: 4 },
  groupWrapper: { marginBottom: 24 },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10, paddingLeft: 2 },
  sectionAccent: { width: 3, height: 18, borderRadius: 4 },
  ...
  zoneSetLabel: { fontSize: 11, fontWeight: '600' },
  inputGroup: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  inputWrapper: {
    borderWidth: 1.5, borderRadius: 12, paddingHorizontal: 12,
    height: 40, width: 64, justifyContent: 'center',
  },
  input: { fontSize: 16, fontWeight: '700', textAlign: 'center' },
  unit: { fontSize: 13, fontWeight: '600' },
  footerNote: { fontSize: 12, fontWeight: '500', textAlign: 'center', marginBottom: 20, paddingHorizontal: 10, lineHeight: 18 },
  saveButton: { shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 12, elevation: 8, borderRadius: 16 },
  saveButtonGradient: { height: 56, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  saveButtonText: { fontSize: 17, fontWeight: '800', letterSpacing: -0.3 },
});
